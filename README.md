...
StockGraph/
├── data/
│   ├── market_overview/      # 每日全市场行情、热度、行业日历 JSON
...
- **A 股中国城市气泡图**：基于 ECharts 中国地图按交易日展示上市公司城市分布，气泡大小关联市值，颜色关联当日涨跌，并复用已有大模型 API 配置做行业新闻关联分析
...
- **全 A 股图谱展示链路已打通**：
  ...
  - 支持按交易日或日期区间查看，并支持行业、交易所、节点类型、关键词和核心节点规模筛选
- **A 股城市气泡图链路已打通**：
  - 统一前端新增「城市气泡图」tab
  - 数据文件：`outputs/app/data/china_city_bubble.json`
  - 股票全集复用 `data/market_overview/stock_daily_*.json`
  - 市值计算复用 `data/reference/stock_basic_info.pkl` 中的总股本/流通股
  - 企业所在地可通过 `scripts/sync_stock_locations.py` 调用 akshare/巨潮公司概况接口增量生成 `data/reference/stock_location_mapping.json`
  - 行业新闻关联复用 `news_articles / news_entities`
  - 支持按交易日或日期区间、省份、城市、行业、市值分层、定位质量和最少股票数筛选
  - 支持图表本地截图，并可选以 OpenAI 兼容 Vision 消息格式随请求发送给大模型
  - 当前基础股本文件没有注册地/办公地字段；若要获得真实城市分布，请补充 `data/reference/stock_location_mapping.json`，格式参考 `data/reference/stock_location_mapping.sample.json`
...
- 新闻和龙虎榜的后端联动分析服务
- 全 A 城市图的精确公司注册地/办公地数据源接入（现已预留映射文件入口，缺失时按交易所所在地降级聚合）
- 图谱社区发现、路径挖掘、主题簇分析
...
python3 scripts/backfill_from_json.py --start-date 2026-04-01 --end-date 2026-04-17
...
`scripts/backfill_from_json.py` 用于不访问网络的历史数据恢复。它读取仓库里已有的 JSON 文件，并把数据写回 SQLite：

- 市场数据来源：`data/market_overview/stock_daily_*.json`、`data/market_overview/hot_daily_*.json`
- 龙虎榜和图快照来源：`outputs/dragon_tiger/dragon_tiger_analysis_*.json`
- 写入表：`market_daily_snapshots`、`market_hot_rankings`、`market_industry_rankings`、`stock_seat_operations`、`daily_summaries`、`seat_details`、`graph_nodes`、`graph_edges`、`graph_snapshots`

常用示例：

```bash
# 先检查可回填行数，不写数据库
python3 scripts/backfill_from_json.py --start-date 2026-05-06 --end-date 2026-06-01 --dry-run

# 回填全部可用 JSON
python3 scripts/backfill_from_json.py

# 只回填龙虎榜
python3 scripts/backfill_from_json.py --areas dragon_tiger

# 只回填市场概览
python3 scripts/backfill_from_json.py --areas market_overview

# 只回填龙虎榜导出 JSON 内的图快照
python3 scripts/backfill_from_json.py --areas graphs

# 回填后重建统一前端，让依赖 SQLite 的展示数据刷新
python3 scripts/build_unified_app.py
```

该脚本不会调用 `sync_dragon_tiger.py` 或 `sync_market_overview.py`，不会触发 akshare/雪球/巨潮等网络抓取。`outputs/app/data/*.json` 属于前端展示产物，不作为数据库回填源。
...
- 同步新闻
- 构建图快照
- 生成统一前端，包括「全 A 图谱」所需的 `stock_super_graph.json` 和「城市气泡图」所需的 `china_city_bubble.json`
- 生成开发首页
...
- `ANALYZE_DRAGON_TIGER=1` — 额外导出龙虎榜分析 JSON，例行任务和回填脚本默认开启
- `SYNC_STOCK_LOCATIONS=1` — 增量同步上市公司所在地。默认关闭，避免每天对巨潮发起大量请求
- `STOCK_LOCATION_LIMIT=100` — 本次最多新增抓取多少只股票，`0` 表示不限制
- `STOCK_LOCATION_SLEEP=1.2` — 每只股票请求后的等待秒数
...
`sync_stock_locations` 用于生成企业真实所在地映射：

- 数据源：`akshare.stock_profile_cninfo`（巨潮资讯公司概况），读取 `注册地址/办公地址`
- 输出：`data/reference/stock_location_mapping.json`。这个文件是地址同步成果，建议提交到 git，后续机器或 GitHub Actions 可在已有成果基础上继续补齐
- 断点缓存：`data/shared_state/stock_location_profile_cache.json`
- 行政区划坐标缓存：`data/reference/china_admin_locations.json`（本地生成，不建议提交）
- 独立批量脚本：`bash scripts/sync_stock_locations_batch.sh`
- 安全建议：首次不要全量跑，先按批次执行，例如 `STOCK_LOCATION_LIMIT=300 STOCK_LOCATION_SLEEP=1.5 bash scripts/sync_stock_locations_batch.sh`；若要继续补齐，重复运行即可跳过已有映射
- 连续跑多批：`STOCK_LOCATION_ROUNDS=5 STOCK_LOCATION_LIMIT=300 STOCK_LOCATION_SLEEP=2 bash scripts/sync_stock_locations_batch.sh`
- 同步后重建前端：`BUILD_APP_AFTER=1 bash scripts/sync_stock_locations_batch.sh`
- 直接 Python 入口：`python3 scripts/sync_stock_locations.py --limit 300 --sleep 1.5`
- 全量更新：`python3 scripts/sync_stock_locations.py --limit 0 --sleep 1.5`
- 只用本地缓存重建映射：`python3 scripts/sync_stock_locations.py --rebuild-only`
- 提交前建议检查 `data/reference/stock_location_mapping.json` 的覆盖率；如果只是部分数据，地图仍会对未覆盖股票使用交易所所在地降级定位
...
- 市值字段由 `最新价 * 总股本` 计算；若总股本缺失，则尝试使用流通股。
- 城市定位优先读取 `data/reference/stock_location_mapping.json`，其次读取未来可能扩展进 `stock_basic_info.pkl` 的省市字段。
- 当前仓库自带的 `stock_basic_info.pkl` 不含注册地/办公地，所以缺失映射时会按交易所所在地降级聚合，并在页面显示定位质量。
- `stock_location_mapping.json` 可使用对象或列表结构，最小字段为 `code/province/city/lng/lat`，样例见 `data/reference/stock_location_mapping.sample.json`。
- AI 行业新闻关联分析复用「AI 分析」tab 的 localStorage 配置，可选择是否把当前 ECharts 截图以 OpenAI 兼容 `image_url` 消息格式附带给支持 Vision 的模型。
- 中国地图边界优先使用本地构建产物 `outputs/app/data/china_echarts_map.js`，源文件为 `data/reference/china_echarts_map.js`（MIT 许可，见同目录 LICENSE），避免浏览器运行时请求 DataV 地图接口时出现 403。
...
- 独立数据文件:
  ...
  - `outputs/app/data/china_city_bubble.json`
...
- `BUILD_GRAPHS=1`
  - 额外执行图快照构建
- `ANALYZE_DRAGON_TIGER=1`
  - 额外导出龙虎榜分析 JSON；可配合 `DRAGON_TIGER_ANALYSIS_START_DATE` / `DRAGON_TIGER_ANALYSIS_END_DATE` 指定区间
- `SYNC_MARKET_OVERVIEW=1`
  - 执行全市场概览和热度同步，默认已开启
...
- 每天 00:30 UTC 定时执行（对应北京时间 08:30），默认处理北京时间前一天的数据
...
定时触发会先通过 akshare A 股交易日历判断“前一天”是否开市。非交易日会正常跳过，不提交数据、不发布 Pages；交易日才会执行例行任务：

1. 同步目标交易日的龙虎榜数据。
2. 导出 `outputs/dragon_tiger/dragon_tiger_analysis_YYYY-MM-DD.json`。
3. 同步 `data/market_overview/stock_daily_YYYYMMDD.json` 和 `hot_daily_YYYYMMDD.json`。
4. 生成行业日历、行业强弱、市场热度、全 A 图谱、城市气泡图等统一前端数据。
5. 构建 GitHub Pages 产物。
6. 将例行产物提交回 GitHub 仓库，再发布 Pages。

例行任务不会同步新闻，也不会运行需要大模型参与的新闻热度/AI 分析能力。前端里的 AI 分析仍保留为浏览器端手动使用。

为降低 akshare 或目标站点封禁风险，Actions 里的默认策略是：

- push 不抓取远端数据。
- 定时任务默认同步龙虎榜、龙虎榜分析、市场概览和图快照。
- 定时任务不默认同步新闻，也不默认构建年度历史榜单。
...
- `target_date`
  - 指定单日例行同步日期，例如 `2026-04-17`。留空时取北京时间前一天
- `backfill_start_date` / `backfill_end_date`
  - 指定历史回填区间。两者同时填写时进入回填模式
- `backfill_areas`
  - 回填功能区，支持 `all` 或逗号分隔的 `dragon_tiger,market_overview,graphs,stock_locations`
- `backfill_from_json`
  - 勾选后使用仓库中已有 JSON 离线回填 SQLite 并重建前端，不重新抓取网络数据；支持 `all`、`dragon_tiger`、`market_overview`、`graphs`
- `force_run`
  - 忽略 A 股交易日历，强制运行目标日期或区间
- `market_overview_year`
  - 额外生成年度行业强弱榜单，例如 `2025`
- `build_graphs`
  - 是否同时执行图快照构建
- `sync_stock_locations`
  - 是否增量同步上市公司所在地。默认关闭；开启后会访问巨潮/akshare
- `stock_location_limit`
  - 所在地同步本次最多新增抓取数量，默认 `100`，`0` 表示不限制
- `stock_location_sleep`
  - 所在地同步每只股票请求间隔秒数，默认 `1.5`
...
```bash
# 判断北京时间前一天是否开市，开市才运行例行任务
python3 scripts/routine_daily_sync.py

# 指定单日，仍会先检查交易日历
python3 scripts/routine_daily_sync.py --target-date 2026-04-17

# 回填一个区间内所有 A 股交易日的龙虎榜和市场概览
python3 scripts/backfill_history.py \
  --start-date 2026-04-01 \
  --end-date 2026-04-17 \
  --areas dragon_tiger,market_overview
...
当前 Pages 采用“**生成产物提交仓库 + artifact 部署**”模式。

这意味着：

- 仓库里保留源码、脚本、配置和文档。
- 定时任务和回填任务会用 `git add -f` 提交被 `.gitignore` 忽略的例行产物。
- 每日市场数据提交到 `data/market_overview/`。
- 龙虎榜分析和报告提交到 `outputs/dragon_tiger/`。
- 统一前端数据提交到 `outputs/app/data/`，页面提交到 `outputs/app/`。
- 市场热度 HTML 提交到 `outputs/market/`，龙虎榜静态页面提交到 `outputs/html/`。
- 上市公司所在地映射提交到 `data/reference/stock_location_mapping.json`，便于后续机器继续补齐。
- SQLite 数据库 `data/shared_state/dragon_tiger.db` 作为运行态文件保留在本地，不随仓库提交；需要恢复时使用 `scripts/backfill_from_json.py` 从已提交 JSON 重建。
